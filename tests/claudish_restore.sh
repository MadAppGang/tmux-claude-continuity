...
#
# Asserts on the EXACT pending-file contents. Panes run `sleep` (not an
# interactive shell), so no precmd hook consumes the pending file — we can read
# back precisely what post_restore wrote.
#
...
# Make a fresh single-pane session named $1 running sleep; echo "w|p|id|cwd|title".
_mkpane() {
  local name="$1"
  _t new-session -d -s "$name" -c /tmp "sleep 600"
