...
for spec in "c:alias whose text names claude" \
  ...
  for _ in 1 2 3 4 5 6 7 8; do [ -s "$TD/launch/$K" ] && break; sleep 0.4; done
...
read -r P K <<<"$(new_pane)"
_tmux send-keys -t "$P" 'svc' C-m 2>/dev/null; sleep 2
[ -s "$TD/launch/$K" ] && ok "recording present while the command RUNS" \
                       || no "recording present while the command RUNS" "absent during svc"

_tmux send-keys -t "$P" C-c 2>/dev/null; sleep 2
...
_tmux send-keys -t "$P" 'echo pretend-terraform-apply' C-m 2>/dev/null; sleep 2
...
read -r P K <<<"$(new_pane)"
for cmd in 'ls' 'pwd' 'echo hi'; do _tmux send-keys -t "$P" "$cmd" C-m 2>/dev/null; sleep 0.6; done
sleep 1
